...
BYTE * JpegFile::JpegFileToRGB(string fileName,
							   UINT *width,
							   UINT *height)

{
	...
	*/
	...
	if ((infile = fopen(fileName.c_str(), "rb")) == NULL) {
		//sprintf(buf, "JPEG :\nCan't open %s\n", fileName);
		//MessageBox(NULL,L"can't open filename");
		return NULL;
	}
...
BOOL JpegFile::GetJPGDimensions(string fileName,
								UINT *width,
								UINT *height)
...
BOOL JpegFile::RGBToJpegFile(string fileName, 
							BYTE *dataBuf,
							UINT widthPix,
							UINT height,
							BOOL color, 
