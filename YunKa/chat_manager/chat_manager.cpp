...
CChatManager::CChatManager()
{
	...
	m_nOnLineStatus = USER_STATUS_OFFLINE;
	m_nOnLineStatusEx = USER_STATUS_OFFLINE;
	m_mapTokens.clear();
	m_mapUsers.clear();
	m_mapWebUsers.clear();	
...
void CChatManager::OnReceiveEvent(int wParam, int lParam)
{
	...
	{
		if (m_nOnLineStatus == USER_STATUS_OFFLINE)
			return;
...
int CChatManager::RecvSrvRepUserinfo(PACK_HEADER packhead, char *pRecvBuff, int len)
{
	...
	if (!m_bLoginSuccess)
	{
		pUser = AddUserObject(RecvInfo.uin, RecvInfo.strid, RecvInfo.UserInfo.info.nickname, USER_STATUS_ONLINE, -1);
...
int CChatManager::RecvSrvStatusFrdOnline(PACK_HEADER packhead, char *pRecvBuff, int len)
{
	...
	else
	{
		...
		if (pUser == NULL)
		{
			...
			pUser->status = USER_STATUS_ONLINE;
	...
	else
	{
		...
		if (pUser != NULL)
		{
			...
			if (pUser->status != USER_STATUS_OFFLINE)
			{
				...
				pUser->talkstatus = TALK_STATUS_NO;
				pUser->status = USER_STATUS_OFFLINE;
			}
...
int CChatManager::RecvComSendMsg(PACK_HEADER packhead, char *pRecvBuff, int len)
{
			...
			else
			{
				...
				if (!(strstr(RecvInfo.msg.strmsg, "[系统消息]") != NULL && strstr(RecvInfo.msg.strmsg, "客人离开") != NULL))
				{
					...
					if (m_sysConfig->m_bAutoRespUnnormalStatus)
					{
						...
						case USER_STATUS_WORK:
						case USER_STATUS_REFUSE_NEWWEBMSG:
							strResp = m_sysConfig->m_sUnnormalStatusMsg;
							break;
						case USER_STATUS_LEAVE:
						case USER_STATUS_EATING:
	...
	else
	{
		...
		if (pUser == NULL)
		{
			pUser = AddUserObject(RecvInfo.msg.senduin, "", RecvInfo.msg.strmobile, USER_STATUS_ONLINE, -1);
			if (pUser == NULL)
			{
				goto RETURN;
			}
		...
		strncpy(pUser->UserInfo.nickname, RecvInfo.msg.strmobile, MAX_USERNAME_LEN);
		...
		if ((int)sMsg.find(m_initConfig.fileserver_media_fileid) > -1)
		{
			...
			AddMsgToList(pUser, msgFrom, MSG_RECV_ERROR, msgId, MSG_TYPE_NORMAL, MSG_DATA_TYPE_IMAGE,
				contentMsg, RecvInfo.msg.sendtime, NULL, NULL, false, true, false);
...
int CChatManager::RecvFloatCreateChat(PACK_HEADER packhead, char *pRecvBuff, int len)
{
	...
	if (RecvInfo.uKefu == 0)
	{
		...
		{
			pWebUser->onlineinfo.talkstatus = TALK_STATUS_REQUEST;
			pWebUser->m_nWaitTimer = 0;
...
int CChatManager::RecvFloatChatInfo(PACK_HEADER packhead, char *pRecvBuff, int len)
{
	...
	if (pWebUser == NULL)
	{
		pWebUser = AddWebUserObject(RecvInfo.strClientId, RecvInfo.strThirdId, RecvInfo.webnickname, "", "", USER_STATUS_UNDEFINE, 0);
		pWebUser->webuserid = RecvInfo.uWebUin;
	}
	else
	{
...
int CChatManager::RecvFloatChatMsg(PACK_HEADER packhead, char *pRecvBuff, int len)
{
	...
	if (packhead.uin != RecvInfo.webuin)
	{
		...
		{
			// 根据uin和昵称先初始化坐席简单信息
			pAssistUser = AddUserObject(packhead.uin, "", RecvInfo.nickname, USER_STATUS_ONLINE, -1);
	...
	case MSG_WX://微信消息 
	{
		...
		//if (m_sysConfig->m_bAutoSearchKeyword)
		{
			...
			//m_pFormKeyWord->ResetKeyWord((m_nOnLineStatus != USER_STATUS_OFFLINE), pUser, RecvInfo.strmsg);
		}
...
void CChatManager::TimerProc(int timeId, LPVOID pThis)
{
	...
	{
		if (chat_manager->m_nOnLineStatus == USER_STATUS_OFFLINE)
			return;
...
void CChatManager::TimerSolveAck()
{
	...
	if (m_nSendPing > 30)
	{
		...
		if (m_nOnLineStatusEx != USER_STATUS_OFFLINE)
		{
...
CWebUserObject * CChatManager::AddWebUserObject(char *sid, char *thirdid, char *name,
	char *scriptflag, char *url, USER_STATUS status, unsigned char floatauth)
{
	...
	pWebUser->info.userstatus = status;
...
WxMsgBase* CChatManager::ParseWxMsg(CWebUserObject* pWebUser, char* msg, CUserObject* pAssistUser, unsigned long time)
{
	...
	if (pwxobj != NULL)
	{
		...
		else if ("image" == pwxobj->MsgType)
		{
			...
			AddMsgToList((IBaseObject*)pWebUser, msgFormType, MSG_RECV_ERROR, msgId, MSG_TYPE_NORMAL, 
				MSG_DATA_TYPE_IMAGE, contentMsg, time, pAssistUser, NULL, false, true, false);
		...
		else if ("voice" == pwxobj->MsgType)
		{
			...
			AddMsgToList((IBaseObject*)pWebUser, msgFormType, MSG_RECV_ERROR, msgId, MSG_TYPE_NORMAL, 
				MSG_DATA_TYPE_VOICE, contentMsg, time, pAssistUser, NULL, false, true, false);
		...
		else if ("video" == pwxobj->MsgType || "shortvideo" == pwxobj->MsgType)
		{
			...
			AddMsgToList((IBaseObject*)pWebUser, msgFormType, MSG_RECV_ERROR, msgId, MSG_TYPE_NORMAL, 
				MSG_DATA_TYPE_VIDEO, contentMsg, time, pAssistUser, NULL, false, true, false);
...
int CChatManager::SendAutoRespMsg(CWebUserObject *pWebUser, const char *msg, BOOL bClearTimer)
{
	...
	{
		AddMsgToList((IBaseObject*)pWebUser, MSG_FROM_SELF, MSG_RECV_ERROR, GetMsgId(), MSG_TYPE_NORMAL, 
			MSG_DATA_TYPE_TEXT,	msg, 0, NULL, NULL);
	...
	{
		AddMsgToList((IBaseObject*)pWebUser, MSG_FROM_SELF, MSG_RECV_ERROR, GetMsgId(), MSG_TYPE_NORMAL,
			MSG_DATA_TYPE_TEXT,	"自动回复消息失败！", 0, NULL, NULL);
	}
...
void CChatManager::SendTo_CloseChat(unsigned long webuserid, int ntype)
{
	...
	if (pWebUser == NULL || pWebUser->onlineinfo.talkstatus == TALK_STATUS_NO)
		return;
...
int CChatManager::SendTo_AcceptChat(unsigned long webuserid)
{
	...
	if (pWebUser)
	{
		...
		{
			AddMsgToList((IBaseObject*)pWebUser, MSG_FROM_SYS, MSG_RECV_ERROR, GetMsgId(), MSG_TYPE_NORMAL, 
				MSG_DATA_TYPE_TEXT,	"该访客已被您屏蔽，请先解除屏蔽后再接受对话请求!", 0, NULL, NULL);
		}
		else
		{
			if (pWebUser->m_bNewComm)
...
int CChatManager::SendTo_InviteWebUser(CWebUserObject *pWebUser, int type, string strText)
{
	...
	if (pWebUser != NULL)
	{
		...
		{
			AddMsgToList((IBaseObject*)pWebUser, MSG_FROM_SYS, MSG_RECV_ERROR, GetMsgId(), MSG_TYPE_NORMAL, 
				MSG_DATA_TYPE_TEXT,"该访客已被您屏蔽，请先解除屏蔽后再邀请!", 0, NULL, NULL);
			return nError;
		}
...
void CChatManager::SetAllUserOffline()
{
	...
	for (iTer_user = m_mapUsers.begin(); iTer_user != m_mapUsers.end(); iTer_user++)
	{
		...
		if (pUserOb != NULL)
		{
			pUserOb->status = USER_STATUS_OFFLINE;
		}
	}
	...
	m_userInfo.status = USER_STATUS_OFFLINE;
}
...
bool CChatManager::ParseTextMsg(CWebUserObject* pWebUser, string content, CUserObject* pAssistUser, unsigned long time)
{
	...
	if ((int)content.find("http:") > -1 && (int)content.find("收到一个图片") > -1 && pAssistUser)
	{
		...
		if (pos > -1 && pos1 > -1)
		{
			...
			AddMsgToList((IBaseObject*)pWebUser, MSG_FROM_ASSIST, MSG_RECV_ERROR, msgId, MSG_TYPE_NORMAL, 
				MSG_DATA_TYPE_IMAGE, contentMsg, 0, pAssistUser, NULL, false, true, false);
		...
		(int)content.find(".jpeg") > -1 || (int)content.find(".bmp") > -1 ||(int)content.find(".png") > -1))
	{
		...
		AddMsgToList((IBaseObject*)pWebUser, MSG_FROM_WEBUSER, MSG_RECV_ERROR, msgId, MSG_TYPE_NORMAL, 
			MSG_DATA_TYPE_IMAGE, contentMsg, 0, pAssistUser, NULL, false, true, false);
...
UINT WINAPI CChatManager::DownLoadFileFromServerThread(void * para)
{
	...
	if (httpLoad.HttpLoad(url, "", REQUEST_TYPE_GET, downPath, rs))
	{
		...
		manager->AddMsgToList(pObj, msgFrom, MSG_RECV_ERROR, msgId, MSG_TYPE_NORMAL, msgDataType, 
			addPath, time, pUser, NULL, true, false);
		return true;
	}
	else
	{
