...
static UINT WINAPI DownLoadFileFromServerThread(void * para)
{
	...
	if (httpLoad.HttpLoad(url, "", REQUEST_TYPE_GET, decodeFilePathDownload, rs))
	{
		...
		{
			// 转换格式
			manager->Amr2Wav(decodeFilePath.c_str());
		}
		manager->m_handlerMsgs->ResultRecvMsg(msgId, true);
		return true;
	}
	else
	{
...
int CChatManager::RecvFloatCreateChat(PACK_HEADER packhead, char *pRecvBuff, int len)
{
	...
	else
	{
		...
		if (pUser != NULL &&(pUser->UserInfo.uid == m_userInfo.UserInfo.uid || pUser->m_bFriend))
		{
			...
			m_handlerMsgs->RecvMsg(pWebUser, MSG_FROM_SYS, GetMsgId(), MSG_TYPE_NORMAL, MSG_DATA_TYPE_TEXT, msg, GetTimeByMDAndHMS(0));
		}
	}
...
int CChatManager::RecvFloatCloseChat(PACK_HEADER packhead, char *pRecvBuff, int len)
{
	...
	else
	{
		...
		m_handlerMsgs->RecvMsg(pWebUser, MSG_FROM_SYS, GetMsgId(), MSG_TYPE_NORMAL, MSG_DATA_TYPE_TEXT, strMsg.c_str(), GetTimeByMDAndHMS(0));
...
WxMsgBase* CChatManager::ParseWxMsg(CWebUserObject* pWebUser, COM_FLOAT_CHATMSG& recvInfo, CUserObject* pUser)
{
	//微信消息类型，utf8编码类型 
	...
	if (pwxobj != NULL)
	{
		...
		else if ("image" == pwxobj->MsgType)
		{
			...
			param->downLoadUrl = ((WxMsgImage*)msgBase)->MediaUrl.c_str();
...
string& CChatManager::TransferFaceToStr(string& msg)
{
	int n;
	...
	for (int i = 0; i < _countof(wx_faces); ++i, ++face)
	{
		for (len = face->raw.length(), pos = 0; begin = msg.find("[", pos), begin != std::string::npos;)
		{
			...
			if ((n = GetFaceIndex(msg.c_str() + begin + 1)) < 0)
				continue;
			const string &face = GetFaceStr(n);
...
const string& CChatManager::GetFaceStr(int id)
{
