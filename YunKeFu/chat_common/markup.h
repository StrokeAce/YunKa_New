// Markup.h: interface for the CMarkupXml class.
//
// Markup Release 10.0
...
public:
	...
	CMarkupXml() { SetDoc( NULL ); InitDocFlags(); };
	CMarkupXml( MCD_CSTR szDoc ) { SetDoc( szDoc ); InitDocFlags(); };
	CMarkupXml( int nFlags ) { SetDoc( NULL ); m_nDocFlags = nFlags; };
	CMarkupXml( const CMarkupXml& markup ) { *this = markup; };
	void operator=( const CMarkupXml& markup );
	~CMarkupXml() {};
