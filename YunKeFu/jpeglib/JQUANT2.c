...
typedef Ushort histcell;	/* histogram cell; prefer an unsigned type */
...
typedef struct {
  ...
  _int_boolean needs_zeroed;		/* TRUE if next pass must zero histogram */

  /* Variables for Floyd-Steinberg dithering */
  FSERRPTR fserrors;		/* accumulated errors */
  _int_boolean on_odd_row;		/* flag to remember which row we are on */
...
METHODDEF(void)
start_pass_2_quant (j_decompress_ptr cinfo, _int_boolean is_pre_scan)
{
  my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo->cquantize;
  hist3d histogram = cquantize->histogram;
  int i;
