...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//初始化
	m_pSendFileBtn = m_pFaceBtn = m_pScreenBtn = pSendMsgBtn = m_pVoiceBtn = NULL;
...
LRESULT CMainFrame::HandleCustomMessage(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if (uMsg == WM_DOUBLE_CLICK_SHOW_WND_MSG)
	{
		m_wndShow = false;
...
void CMainFrame::OnButtonDoChanged(TNotifyUI &msg)
{
	...
	else if (msg.pSender->GetName() == _T("chat_list_button"))
	{
	...
	else if (msg.pSender->GetName() == _T("friend_list_button"))
	{
		pTabControl = static_cast<CTabLayoutUI*>(m_PaintManager.FindControl(_T("left_button_tab_frame")));
		if (pTabControl != NULL)
		{
...
UserListItemInfo *CMainFrame::GetOneUserItemInfo(unsigned long uid, string sid)
{
	...
	if (uid != 0)
	{
		iter = m_pMyselfList.begin();
		for (; iter != m_pMyselfList.end(); iter++)
		{
			if ((*iter)->_uid == uid)
			{
				info = *iter;
		...
		iter = m_pWaitList.begin();
		for (; iter != m_pWaitList.end(); iter++)
		{
			if ((*iter)->_uid == uid)
			{
				info = *iter;
...
void CMainFrame::RemoveOneUser(UserListItemInfo* getInfo)
{
	...
	if (getInfo->type == 0)
	{
		pWaitChatList->Remove((CControlUI*)getInfo->plistElement);
		...
		for (; iter != m_pWaitList.end(); iter++)
		{
			if ((*iter) == getInfo)
			{
				m_pWaitList.remove(*iter);
				break;
			}
		}
	}
	else if (getInfo->type == 1)
	{
...
void CMainFrame::RecvAcceptChat(CWebUserObject* pWebUser, CUserObject* pUser)
{
	...
	UserListItemInfo *info = GetOneUserItemInfo(pWebUser->webuserid, pWebUser->info.sid);

	if (info != NULL)
	{
		if (pUser == NULL)
		{
			OnMoveUserPos(info, -1);
			return;
		}
...
void CMainFrame::OnActiveUser(UserListItemInfo *info)
{
	...
	if (info->type == 0)
	{
		...
		m_manager->SendTo_AcceptChat(info->_uid);
...
void CMainFrame::OnItemClickEvent(unsigned long id, int type)
{
	...
	if (m_curSelectId != id || type == 0 || (id == 0 && m_curSavedSid.length() > 0))
	{
...
void CMainFrame::OnClick(TNotifyUI& msg)
{
	...
	if (msg.pSender->GetName() == _T("mid_button_1")) //接受
	{
		AcceptChat();
	}
